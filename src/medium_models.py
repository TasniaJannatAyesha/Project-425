...
class HybridConvVAE(nn.Module):
    def __init__(self, audio_dim, lyric_dim, latent_dim=32):
        ...
        # 1. Audio Encoder (Convolutional)
        # We treat MFCCs as a sequence that the CNN can "slide" over
        ...
        # 2. Hybrid Projection (Combining Audio + Lyrics)
        # 16 filters * (audio_dim/2) + lyric_dim
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(128, audio_dim + lyric_dim) # Reconstruct both
        )
    ...
    def forward(self, audio, lyrics):
        # Audio needs a 'channel' dimension for Conv1d: (batch, 1, audio_dim)
        a_feat = self.audio_conv(audio.unsqueeze(1))
        
        # Concatenate audio features with lyric embeddings
